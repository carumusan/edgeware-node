...
use srml_support::{decl_module, decl_storage, traits::Randomness};
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		fn on_initialize(block_number: T::BlockNumber) {
			let parent_hash = <srml_system::Module<T>>::parent_hash();
...
impl<T: Trait> Randomness<T::Hash> for Module<T> {
	...
	fn random(subject: &[u8]) -> T::Hash {
		let block_number = <srml_system::Module<T>>::block_number();
		let index = block_number_to_index::<T>(block_number);
...
mod tests {
	...
	fn new_test_ext() -> runtime_io::TestExternalities {
		let t = srml_system::GenesisConfig::default().build_storage::<Test>().unwrap();
		t.into()
	}
